...
\addcontentsline{toc}{section}{CONTEXT SI MOTIVATIE}
  ...
  Momentan, in domeniul modelelor de limbaj, exista cativa jucatori mari precum \href{https://www.google.com}{Google}, \href{https://www.anthropic.com}{Anthropic} si \href{https://openai.com}{OpenAI} care controleaza o buna parte
...
\chapter{LUCRARI ANTERIOARE}
  ...
  Desi entuziasmul pentru bula \ac{ia} atinge cote maxime, inca este mult loc de imbunatatire, iar aceste platforme de asistenti de programare au inca
  ...
  In final am ales sa folosesc \textbf{LangGraph} datorita controlului mare pe care il ofera asupra desfasurarii workflow-ului agentic si a integrarii cu \href{https://langfuse.com}{Langfuse}
  \footnote{Unealta de observabilitate open-source pentru agenti, disponibila la \url{https://langfuse.com}}
  ...
  \vspace{2mm}
    \begin{tabularx}{\textwidth}{@{} >{\hsize=0.55\hsize\RaggedRight}X >{\hsize=1.15\hsize\RaggedRight}X >{\hsize=1.15\hsize\RaggedRight}X >{\hsize=1.15\hsize\RaggedRight}X @{}}
      ...
      Motor \ac{tui} pe ecran complet folosind buffer-ul alternativ al terminalului. Arbore de widget-uri bazat pe \ac{dom} (similar cu \ac{html}) &
      $\bullet$ Sistem de layout similar cu \ac{css} (grid, dock)\newline
...
\chapter{PRACTICI CURENTE IN INDUSTRIE}
  ...
  \section{Aplicatii \ac{oss} concurente}
  ...
  personalizare si consistenta conversatiei.
    ...
    \begin{itemize}
      \item Utilizeaza doua locuri de stocare a memoriei: stocare intr-un folder ascuns a conversatiilor integrale si in memorie pentru istoricul conversatiei compactate (daca este cazul)
    ...
    \end{itemize}
      ...
      Data fiind alegerea de abandonare a tehnicii \ac{rag}, in implementare avem nevoie de o modalitate inteligenta de a prelucra datele multimodale pentru prezevarea contextului.
      ...
      agnostice de furnizor care permite folosirea oricarui model, avem nevoie sa detectam capabilitatile de prelucrarea a tipurilor de fisiere pentru fiecare model in parte. Din pacate,
      furnizorii de modele sau creatorii care le includ in Ollama, nu specifica nicaieri consistent in metadatele modelului daca acesta capabilitati pentru lucrul cu anumite tipuri de fisiere.
...
\chapter{REZULTATE EXPERIMENTALE}
    ...
    Dat fiind natura nedeterminista a raspunsurilor modelelor de limbaj, schimbarile de configuratie precum modelul utilizat sau promptul de sistem,
  ...
  \section{Pozitionarea fata de concurenta}
    ...
    scapand de dependenta de un furnizor de modele de limbaj scapand astfel de costurile asociate cu folosirea acestor unelte, transparenta interactiunii cu modelele de limbaj prin
    ...
    \textit{\href{https://github.com/openai/codex}{OpenAI Codex \ac{cli}}} dezvoltata de OpenAI
    si \textit{\href{https://docs.github.com/en/copilot/concepts/agents/about-copilot-cli}{GitHub Copilot \ac{cli}}} dezvoltata de GitHub) in raport cu solutia propusa.
    ...
    \setlength{\tabcolsep}{4pt}
    \renewcommand{\arraystretch}{1.5}
    \caption{Comparatie intre \texttt{vibe-cli} si principalele unelte agentice de programare pentru terminal disponibile pe piata}
    \vspace{2mm}
      \begin{tabularx}{\textwidth}{@{} >{\hsize=0.55\hsize\RaggedRight}X >{\hsize=0.95\hsize\RaggedRight}X >{\hsize=1.25\hsize\RaggedRight}X >{\hsize=1.25\hsize\RaggedRight}X @{}}
        ...
        $\bullet$ \textbf{Panou web Flask local}, read-only, cu metrici per sesiune (tokeni pe categorii, evolutia contextului, cost estimat, latenta uneltelor cu $p_{50}$/$p_{95}$, debit, compactari, reconstructia turului)
    ...
    In tabelul \ref{tab:comparatie_clis}, se observa mai multe diferente, 
    ...
    \setlength{\tabcolsep}{4pt}
    \renewcommand{\arraystretch}{1.5}
    \caption{Comparatie intre \texttt{vibe-cli} si principalii asistenti de programare \ac{oss} din ecosistem}
    \vspace{2mm}
      \begin{tabularx}{\textwidth}{@{} >{\hsize=0.55\hsize\RaggedRight}X >{\hsize=0.95\hsize\RaggedRight}X >{\hsize=1.25\hsize\RaggedRight}X >{\hsize=1.25\hsize\RaggedRight}X @{}}
        \toprule
        \textbf{Aplicatie} & \textbf{Furnizori si model de cost} & \textbf{Puncte forte} & \textbf{Puncte slabe (raportat la solutia propusa)} \\
        ...
        \textbf{Cline} \newline (\href{https://github.com/cline/cline}{GitHub}) \newline Apache 2.0 &
        Agnostic, cu rutare prin \href{https://openrouter.ai}{OpenRouter} (200+ modele: Anthropic, OpenAI, Ollama). \newline
        ...
        \textbf{Aider} \newline (\href{https://github.com/Aider-AI/aider}{GitHub}) \newline Apache 2.0 &
        Agnostic (Claude, \ac{gpt}, \href{https://www.deepseek.com}{DeepSeek}, Ollama). \newline
        Aplicatie gratuita; cost limitat la tariful modelului ales. &
    ...
    si functionalitati genuin absente.
    ...
    \begin{multicols}{2}
      \begin{enumerate}
        ...
        \item Permisiuni \textit{allow}/\textit{deny} per unealta
        \item Sandboxing optional in container Docker
        \item Comenzi cu autocompletare
        \item Sesiuni persistente cu \textit{save}/\textit{resume}
    ...
    \begin{enumerate}
      ...
      \begin{multicols}{2}
        \begin{enumerate}
          \item Aplicatie nativa desktop sau mobila sau extensii \ac{ide}
    ...
    \item \textbf{Existente indirect, prin mecanisme deja prezente:}
      ...
      \begin{enumerate}
        ...
        \item Integrariea cu GitHub pentru \ac{pr}-uri sau GitHub Issues este realizata prin servere \ac{mcp} sau invocarea utilitarului \texttt{gh}
  ...
  \section{Influenta lungimii si a exactitatii prompt-ului asupra performantei. De ce un prompt prea exact nu garanteaza rezultate mai bune}
    ...
    Unul dintre experimentele rulate a fost evaluarea performantelor agentului in sarcini de \ac{pr} review pe 6 proiecte open-source cu doua tipuri de prompt de sistem:
    ...
    instantele ce trebuiesc recunoscute fara a returna rezultate ireale. In domenii sensibile precum domeniul medical sau militar, acest subiect este foarte dezbatut, deoarece in general
    daca incercam sa marim recunoasterea, precizia va scadea. O alternativa este folosirea scorului F1, dar nici aceasta metoda nu este perfecta. 
...
\chapter{DEZVOLTARI VIITOARE}
  ...
  \section{Suport pentru metode de compactare avansate si \ac{sdk} pentru agenti}

  In final, o alta potentiala aditie viitoare ar fi cea legata de tehnicile avansate de compactare bazate pe similaritatea semantica a frazelor ce urmeaza a fi sters din cauza
  criteriilor de similaritate sau din cauza categorisirii ca fiind neimportante. Aceste tehnici pot preveni stergerea acestora daca sunt considerate informatii vitale.
...
\chapter{CONCLUZII}
  ...
  costurile reduse de operare, impuse doar de utilizarea unor modele proprietare. De asemenea, pe langa echivalarea capabilitatilor aplicatilor din industrie si costurile reduse, am reusit sa aducem transparenta in utilizarea
  modelelor de limbaj pentru utilizatori ce doresc sa studieze cum functioneaza acestea in realitate in domeniul dezvoltarii software in vederea optimizarii si imbunatatirii sistemelor agentice sau modului in care folosim astfel de aplicatii si sa 
  extragem un set de recomandari in vederea utilizarii aplicatiilor agentice in scopul dezvoltarii software in urma experimentelor pe care le-am condus.
  ...
  de pe urma ajutorului pe care aceste platforme il pot oferi. Din punctul de vedere al cercetarilor lucrarilor din domeniu, nici acest lucru nu a reprezentat un impediment datorita valului de studii si dezvoltari in aceasta zona care creste de pe o zi pe alta.
  In final, pentru studiul pietei am reusit sa sondez majoritatea solutiilor deja existente, folosindu-le in sarcinile de zi cu zi, pentru zone pe care acestea nu le ating. In ciuda diversitatii si a capabilitatilor avansate, tuturor acestora le lipsea cu desavarsire compatibilitatea
  ...
  Trecand la partea practica a lucrarii, am intampinat un numar de obstacole. Desi planul era facut, am invatat ca rareori acesta ramane neschimbat pe parcursul implementarii. De la decizia de a schimba
  ...
  implementarea datelor de intrare multimodale care a fost ultima schimbare majora.

  Dupa mai multe runde de imbunatatiri, am constatat concretizarea proiectului intr-un produs consistent. Au urmat apoi partea de gandire a procesului de instalare si finisare a proiectul dar si scrierea de documentatie.

  In final, avand un rezultat finisat, studiul utilizarii acestuia putea incepe pentru a putea trage concluzii cu privire la eficacitatea acestuia in contextul actual.

  Cu datele din aceste studii, am reusit sa formulez o serie de recomandari pentru utilizarea eficienta a aplicatiilor agentice in domeniul dezvoltarii software.

  Insumand toate acestea, in final, prin solutia prezentata am demonstrat valoarea arhitecturilor agentice in domeniul dezvoltarii software, care prompt sa schimbe complet
